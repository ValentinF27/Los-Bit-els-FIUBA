  ...
  partituras.forEach(p => {
    ...
    contenedor.innerHTML += `
      <div class="column is-one-quarter">
        <a href="partitura.html?id=${p.id}" class="card">
        <div class="card">
          <div class="card-image">
            <figure class="image is-4by3">
              <img src="${p.imagen || './iconos/partitura.png'}" alt="Vista previa de la partitura">
            </figure>
          </div>
          <div class="card-content">
            <p class="title is-5">${p.nombre}</p>
            <p class="subtitle is-6">${p.instrumento} - ${p.artista}</p>
